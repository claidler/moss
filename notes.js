...
async function ingestCron() {
  ...
  try {
    ...
    for (const job of jobs.slice(0, 50)) {
      ...
      try {
        ...
        for (const run of entries.slice(0, 4)) {
          ...
          const kind = store.upsertNote(notes, full);
          if (kind) changed = true;
          if (kind === "new") fresh.push(store.noteIdFor(full));
        }
      } catch (e) {
    ...
    fresh.forEach((id) => {
      ...
      if (item) push.notifyAutomation(item);
    });
  } catch (e) {
...
function announceNew(kind, run) {
  if (kind !== "new" || !run) return;
  const item = (store.loadNotes().items || []).find((n) => n && n.id === store.noteIdFor(run));
  if (item) push.notifyAutomation(item);
}
...
function scheduleRetry(run) {
  ...
  setTimeout(() => {
    enrich.enrichRun(run)
      .then((full) => {
        announceNew(store.ingestRun(full), full);
      })
      .catch(() => {})
      .finally(() => {
        retrying.delete(id);
...
function handleCronEvent(msg) {
  ...
  enrich.enrichRun(run)
    .then((full) => {
      const kind = store.ingestRun(full);
      if (kind) console.log("moss-notes live", full.jobName, String(full.summary || "").slice(0, 60));
      announceNew(kind, full);
      if (text.looksPartial(full.summary || full.error || "")) scheduleRetry(full);
    })
    .catch(() => {
      announceNew(store.ingestRun(run), run);
      scheduleRetry(run);
    });
}
...
async function api(req, res) {
  ...
  if (!id && extra === undefined && req.method === "POST") {
    ...
    const kind = store.ingestRun(run);
    announceNew(kind, run);
  ...
  if (id && req.method === "GET") {
    ...
    const row = store.slimNote(filled, true);
    try {
      const thread = (chatStore.loadStore().chats || []).find((c) => c && c.noteId === id);
      if (thread) row.threadChatId = thread.id;
    } catch (e) {}
    json(res, 200, row);
    return true;
  }
