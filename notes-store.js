...
const { OWNER_HANDLE } = require("./config");

const OWNER_MENTION = OWNER_HANDLE ? new RegExp("[ \\t]*@" + OWNER_HANDLE + "\\b", "gi") : null;
...
function noteFromRun(run) {
  let content = String(run.error || run.summary || "");
  if (OWNER_MENTION) content = content.replace(OWNER_MENTION, "");
