...
function renderModelMenu(chat) {
  ...
  search.addEventListener("input", () => {
    ...
    renderModelMenu(chatById(state.activeId) || chat);
  ...
  if (modelPick) {
    modelPick.addEventListener("toggle", () => {
      if (modelPick.open) {
        ...
        renderModelMenu(chatById(state.activeId));
  ...
  if (thinkPick) {
    thinkPick.addEventListener("toggle", () => {
      if (thinkPick.open) {
        ...
        renderThinkMenu(chatById(state.activeId));
      }
    });
  }
...
export async function bootModels() {
  bindPickers();
  syncModelBar(chatById(state.activeId));
  try {
