...
export function paintThinking(el, parts, liveOpen, openOverride) {
  if (!el) return;
  ...
  // Live DOM wins; on a fresh rebuild an explicit override from the caller
  // beats the default, else live runs paint open.
  const wasOpen = prev
    ? Boolean(prev.open)
    : (openOverride == null ? Boolean(liveOpen) : Boolean(openOverride));
  el.innerHTML = "";
...
// Snapshot which tool/thinking <details> the user has open, per bot message,
// so a full log rebuild (1s poll repaint) can restore them instead of closing
// a tool list the user is reading mid-run.
export function captureGroupOpenStates(rootEl) {
  const out = [];
  (rootEl || document).querySelectorAll(".msg.bot").forEach((rowEl) => {
    const bubble = rowEl.querySelector(".bubble");
    const entry = {};
    if (bubble) {
      const tg = bubble.querySelector(".tools details.tool-group");
      if (tg) {
        const items = [];
        tg.querySelectorAll("details.tool").forEach((d) => items.push(d.open));
        entry.tools = { open: tg.open, items };
      }
      const th = bubble.querySelector(".think details.think-group");
      if (th) entry.thinking = th.open;
      const mdel = bubble.querySelector(".md");
      if (mdel && mdel.dataset.htmlView) entry.mdView = mdel.dataset.htmlView;
    }
    out.push(entry);
  });
  return out;
...
export function paintTools(rowEl, tools, openState) {
  if (!rowEl) return;
  ...
  const wasOpen = prevGroup
    ? Boolean(prevGroup.open)
    : Boolean(openState && openState.open);
  const prevToolOpen = [];
  if (prevGroup) {
    prevGroup.querySelectorAll("details.tool").forEach((d, i) => { prevToolOpen[i] = d.open; });
  } else if (openState && Array.isArray(openState.items)) {
    openState.items.forEach((v, i) => { prevToolOpen[i] = Boolean(v); });
  }
  rowEl.innerHTML = "";
