...
const { GW, OPENCLAW_DIR, AGENT_ID } = require("./config");
...
function hubCovers(scopes) {
  ...
  const have = new Set(hub.scopes || []);
  return (Array.isArray(scopes) ? scopes : []).every((s) => have.has(s));
}
...
async function gatewayRpc(scopes, method, params, timeoutMs) {
  ...
  if (hubCovers(scopes)) return wait(hub.rpc);
...
async function abortChatRun(chatId) {
  ...
  try {
    try {
      return await timed(
        h.rpc("sessions.abort", { key: sessionKey, agentId: AGENT_ID, clearQueued: true }),
        12000,
        "sessions.abort"
      );
    } catch (e) {
      console.log("moss-stop sessions.abort", e && e.message);
      return await timed(
        h.rpc("chat.abort", { sessionKey, agentId: AGENT_ID }),
        8000,
        "chat.abort"
      );
    }
